---
title: "Exercise 2: Understanding Concepts"
...
7. **External types**. The User and Item types are external types, specified as generic parameters of the concept. The Item type might be populated by SKU codes, for example. Explain why this is preferable to representing items with their names, descriptions, prices, etc.
...
2. Write a where/then specification for each of the two actions. (_Hints_: The register action creates and returns a new user. The authenticate action is primarily a guard, and doesn't mutate the state.)
...
In this exercise, you'll define concepts for some examples that you're probably familiar with. For each one, you should provide all the standard elements of a concept (name, purpose, operational principle, type declarations, state and actions), and explain any subtleties of your definition in a brief notes section at the end.
...
Define a concept for the shopping cart of an online store. Include only the shopping behavior itself, and not the all the other functions that an online store typically supports with other concepts (such as searching and paying). Even ordering is a distinct concept, and you can assume that when the checkout action occurs in the shopping cart concept, a corresponding order is created in the ordering concept (by a reaction that you do not need to specify).
...
**Hints**: TOTP is not the same as the scheme in which a token is sent by email or text message; an important property of TOTP is that there is no need to send any token to the user at authentication time. You can find an algorithmic outline of TOTP on [Wikipedia](https://en.wikipedia.org/wiki/Time-based\_one-time\_password), but note that you should not include in your concept any cryptographic details, but can just assume the existence of a cryptographic function that you can refer to in your concept specification. The state of your concept should include both state that belongs on the server and state that belongs in users' client machines; make clear in your concept definition where each component of the state belongs. 

**Deliverables**: three concept specifications, with any subtleties explained in brief additional notes.

## Advice

- **Concept guides**. [Defining a concept](../background/defining-concepts.md) explains how to develop a concept definition. Use [Specifying a concept](../background/specifying-concepts.md) for the detailed notation and [Specifying concept state](../background/specifying-concept-state.md) for state declarations.
